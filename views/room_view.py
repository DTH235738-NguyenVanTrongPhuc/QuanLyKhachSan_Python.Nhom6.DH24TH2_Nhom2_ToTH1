...
class RoomView:
    ...
    def create_widgets(self):
        ...
        self.cbb_trangthai = ttk.Combobox(frame_info, 
        ...
        # Treeview with room type information
        ...
        for i, (text, command, color) in enumerate(buttons):
            ...
            # Disable edit/delete for non-managers if needed
    ...
    def add_room(self):
        ...
        try:
            ...
            cur.execute("INSERT INTO phong (maphong, maloai, trangthai, ghichu) VALUES (%s, %s, %s, %s)",
                       (self.entry_maphong.get(), maloai,
    ...
    def edit_room(self):
        ...
        self.entry_maphong.config(state="disabled")  # Cannot change room code
        
        # Set room type
        room_type_display = f"{v[1]}"
        self.cbb_loaiphong.set(room_type_display)
    ...
    def save_room(self):
        try:
            ...
            cur.execute("UPDATE phong SET maloai=%s, trangthai=%s, ghichu=%s WHERE maphong=%s",
                       (maloai, self.cbb_trangthai.get(),
                        self.entry_ghichu.get(), self.entry_maphong.get()))
            self.db.commit()
            self.load_data()
            self.clear_form()
    ...
    def delete_room(self):
        ...
        cur = self.db.get_cursor()
        cur.execute("SELECT COUNT(*) FROM datphong WHERE maphong=%s AND trangthai='Active'", (maphong,))
        if cur.fetchone()[0] > 0:
